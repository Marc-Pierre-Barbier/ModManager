...
fn main() {
	let steam_appid_string = env::args().rfind(|f| f.contains("AppId=")).expect("[MOD_MANAGER] This is not steam, We are expecting to see AppId=VALUE in the args");
	...
	let game_path_index = env::args().position(|f| f == "waitforexitandrun").expect("[MOD_MANAGER] Failed to find GamePath index") + 1;
	...
	let mut child_args: Vec<_> = env::args().skip(1).collect(); //skip ourselves
	child_args[game_path_index - 1] = new_path.to_str().expect("[MOD_MANAGER] Failed to process game args").to_string();
