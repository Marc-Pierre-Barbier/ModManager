...
extern "C" {
	fn _deploy(appid: std::os::raw::c_int) -> DeploymentErrors_t;
	fn _undeploy(appid: std::os::raw::c_int) -> error_t;
	fn _is_deployed(appid: std::os::raw::c_int, status: * mut bool) -> error_t;
	fn _get_deploy_target(appid: std::os::raw::c_int) -> * mut std::os::raw::c_char;
	fn get_game_executable(appid: std::os::raw::c_int) -> * mut std::os::raw::c_char;
}

pub fn bind_deploy(appid: i32) -> DeploymentErrors_t {
	unsafe {
		return _deploy(appid);
	}
}

pub fn bind_undeploy(appid: i32) -> bool {
	unsafe {
		return _undeploy(appid) == error_t::ErrSuccess;
	}
}
...
pub fn bind_is_deployed(appid: i32) -> Option<bool> {
	...
	unsafe {
		let error = _is_deployed(appid, &mut status);
...
pub fn bind_get_deploy_target(appid: i32) -> std::path::PathBuf {
	let path:String;
	unsafe {
		let c_path = _get_deploy_target(appid);
